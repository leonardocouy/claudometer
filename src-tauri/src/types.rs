...
impl ClaudeUsageSnapshot {
  ...
  pub fn last_updated_at(&self) -> &str {
    match self {
      Self::Ok { last_updated_at, .. } => last_updated_at,
      Self::Unauthorized { last_updated_at, .. } => last_updated_at,
      Self::RateLimited { last_updated_at, .. } => last_updated_at,
      Self::Error { last_updated_at, .. } => last_updated_at,
      Self::MissingKey { last_updated_at, .. } => last_updated_at,
    }
  }
}
...
impl<T> IpcResult<T> {
  ...
  pub fn err(code: impl Into<String>, message: impl Into<String>) -> Self {
    Self::Err {
      ok: false,
      error: IpcError { code: code.into(), message: message.into() },
    }
  }
}
